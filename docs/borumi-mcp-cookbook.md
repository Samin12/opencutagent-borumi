# Borumi MCP cookbook (verified 2026-09-15 on Borumi 0.30.5, live smoke project)

Every fact below was executed against a real project through `borumi mcp`, not read from docs.

## Connection and gating
...
- Guides gate tools PER CONNECTION. `get_guides` with no args first, then one id per call. `export_video` refuses with `guide_required` until `exporting` was fetched in that connection; `add_segments` needs `editing_segment_<type>_add`; `create_scenes` needs `scripting`. Fetch the guide, retry.
...
## The pinned-camera layout (Samin's measured treatment, verified render)
```json
...
## Overlays (render ABOVE the camera)
...
- `text_overlay` chapter card that matches the Astra edit: DM Sans, bold, size 62, `sizing_mode:"fixed"`, position custom x 0.055 y 0.76 w 0.7 h 0.16, alignment left, background rgba(17,31,47,239), paragraph sizing, padding 0.3/0.22, radius 0.016, entrance fade, exit instant. Verified render.
...
## Transcripts (verified on the narrated test project, Borumi's local engine)
...
## Layering, splitting, ripple (probed in aborted transactions on the smoke project)
